...
#define nrn_init _nrn_init__NetStimNEWpud
#define _nrn_initial _nrn_initial__NetStimNEWpud
#define nrn_cur _nrn_cur__NetStimNEWpud
#define _nrn_current _nrn_current__NetStimNEWpud
#define nrn_jacob _nrn_jacob__NetStimNEWpud
#define nrn_state _nrn_state__NetStimNEWpud
#define _net_receive _net_receive__NetStimNEWpud 
#define init_sequence init_sequence__NetStimNEWpud 
#define next_invl next_invl__NetStimNEWpud 
#define noiseFromRandom noiseFromRandom__NetStimNEWpud 
#define seed seed__NetStimNEWpud 
...
#define erand erand_NetStimNEWpud
#define invl invl_NetStimNEWpud
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 1;
 	...
 	ivoc_help("help ?1 NetStimNEWpud /home/ifbdtk/LUT_SCI_Hunter/LUT_Baseline/biophys_components/mechanisms/modfiles/x86_64/netstimNEWpud.mod\n");
...
static int next_invl(_threadargsprotocomma_ double);
...
static int  next_invl ( _threadargsprotocomma_ double _lt ) {
   if ( number > 0.0 ) {
     event = invl ( _threadargscomma_ interval - 1.616166e-3 * _lt ) ;
     }
   if ( ispike >= number ) {
     on = 0.0 ;
     }
...
static double _hoc_next_invl(void* _vptr) {
 double _r;
  ...
  _nt = (_NrnThread*)((Point_process*)_vptr)->_vnt;
 _r = 1.;
 next_invl ( _p, _ppvar, _thread, _nt, *getarg(1) );
 return(_r);
}
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/ifbdtk/LUT_SCI_Hunter/LUT_Baseline/biophys_components/mechanisms/modfiles/netstimNEWpud.mod";
static const char* nmodl_file_text = 
  ": $Id: netstim.mod 2212 2008-09-08 14:32:26Z hines $\n"
  ": comments at end\n"
  "\n"
  "NEURON	{ \n"
  "  ARTIFICIAL_CELL NetStimNEWpud\n"
  ...
  "\n"
  "PARAMETER {\n"
  "	interval	= 1000 (ms) <1e-9,1e9>: time between spikes (msec)\n"
  "	number	= 1000000 <0,1e9>	: number of spikes (independent of noise)\n"
  ...
  "}\n"
  "\n"
  "FUNCTION invl(mean (ms)) (ms) {	:implement Poisson distribution - negative exponential\n"
  "	if (mean <= 0.) {\n"
  ...
  "}\n"
  "\n"
  "PROCEDURE next_invl(t(ms)) {       		:CHANGED FROM ORIGINAL FILE\n"
  "	if (number > 0) {\n"
  "		event = invl(interval-1.616166e-3*t) :CHANGED(1-33 Hz, i.e., 1000 to 30.30 ms, in 10 min)\n"
  "	}\n"
  "	if (ispike >= number) {\n"
  "		on = 0\n"
  "	}\n"
  ...
  "			next_invl(t)   :CHANGED FROM ORIGINAL FILE\n"
  "			event = event - interval*(1. - noise)\n"
  "			net_send(event, 1)\n"
  ...
  "	if (flag == 1 && on == 1) {\n"
  "		ispike = ispike + 1\n"
  "		net_event(t)     : notifies all netcons for which this is source that it fired\n"
  "		next_invl(t)     :CHANGED FROM ORIGINAL FILE\n"
  "		if (on == 1) {\n"
  "			net_send(event, 1)\n"
  "		}\n"
