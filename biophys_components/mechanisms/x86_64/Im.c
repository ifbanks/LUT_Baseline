...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 im /home/ifbdtk/LUT_SCI_Hunter/LUT_Baseline/biophys_components/mechanisms/modfiles/x86_64/Im.mod\n");
...
double alf ( _threadargsprotocomma_ double _lv ) {
   double _lalf;
  _lalf = 0.016 / exp ( - ( _lv + vhalf ) / 25.0 ) ;
...
double bet ( _threadargsprotocomma_ double _lv ) {
   double _lbet;
  _lbet = 0.016 / exp ( ( _lv + vhalf ) / 12.0 ) ;
...
static int  rate ( _threadargsprotocomma_ double _lv ) {
  ...
  _laa = alf ( _threadargscomma_ _lv ) ;
   ...
   if ( _lv < - 50.5 ) {
     ...
     ninf = 1.0 / ( 1.0 + exp ( ( - _lv - vhalf ) / 10.34 ) ) ;
     }
   taun = 1.0 / _lsum ;
     return 0; }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/ifbdtk/LUT_SCI_Hunter/LUT_Baseline/biophys_components/mechanisms/modfiles/Im.mod";
static const char* nmodl_file_text = 
  ...
  "	UNITSOFF\n"
  "	alf = 0.016/exp(-(v+vhalf)/25) : 23\n"
  "	UNITSON\n"
  "}\n"
  "\n"
  "FUNCTION bet(v (mV)) (/ms) {\n"
  "	UNITSOFF\n"
  "	bet = 0.016/exp((v+vhalf)/12) :18.8\n"
  "	UNITSON\n"
  "}\n"
  "\n"
  "PROCEDURE rate(v (mV)) {\n"
